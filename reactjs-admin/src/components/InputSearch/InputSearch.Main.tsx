  ...
  return (
    <div 
        className={`flex justify-between items-center space-x-1 rounded border-[1.5px] border-stroke bg-transparent 
            ...
            disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary ${isFocusInput ? 'border-primary dark:border-primary' : ''}
            ${loading ? 'pointer-events-none opacity-50' : ''}
            `}
    >
        <div className='flex-shrink-0 w-[40px] h-[40px] flex justify-center items-center'>
            {
                (isSearchText || loading) && <div className="h-5 w-5 animate-spin rounded-full border-2 border-solid border-sky border-t-transparent"></div>
                || <CiSearch 
