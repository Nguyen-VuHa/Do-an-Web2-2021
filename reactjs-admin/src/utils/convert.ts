...
export function base64ToFileWithMime(base64String: string, fileName: string): File {
  ...
  return new File([blob], removeDiacriticsAndToLowerCase(fileName), { type: mimeType });
}


function removeDiacriticsAndToLowerCase(str: string): string {
  const from = 'àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđĐ'.split('');
