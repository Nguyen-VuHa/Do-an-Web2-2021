...
const CrawlFileModal = () => {
  const {
    isProcessCrawlFile,
    isUploadFileCrawler,
    fileCrawler,
    reqCrawlMovieDetailByFile,
    setDataKeyValue,
  ...
  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'text/plain') {
      setDataKeyValue('fileCrawler', file);
    } else {
      toast.error('Chỉ chấp nhận file .txt, vui lòng chọn đúng loại tệp');
    }
  };

  const handleCloseModal = () => {
    setDataKeyValue('fileCrawler', null);
    setDataKeyValue('isUploadFileCrawler', false);
  };

  const handleCrawlFile = () => {
    if (fileCrawler) {
      reqCrawlMovieDetailByFile(fileCrawler);
    } else {
  ...
  return (
    <Modal
      ...
      isOpen={isUploadFileCrawler}
      onClose={() => {
        if (!isProcessCrawlFile) handleCloseModal();
      }}
      onSubmit={() => {
        if (!isProcessCrawlFile) handleCrawlFile();
      }}
      widthClass="w-[40%]"
      isLoading={isProcessCrawlFile}
    >
      <div className="relative block w-full cursor-pointer appearance-none rounded border-2 border-dashed border-primary bg-gray py-4 px-4 dark:bg-meta-4 sm:py-7.5">
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          {(fileCrawler && (
            <span className="text-warning">{fileCrawler.name}</span>
          )) || (
            <span className="text-primary">Nhấn để tải tệp từ máy tính</span>
          )}
          <span className="text-rose italic text-sm">
            Chỉ chấp nhận file .txt và chứa các URL thu thập dữ liệu
          </span>
        </div>
      </div>
    </Modal>
